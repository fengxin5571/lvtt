...
{
	return $GLOBALS['db']->getRow('SELECT cat_name, cat_alias_name, keywords, cat_desc, style, grade, filter_attr, parent_id FROM ' . $GLOBALS['ecs']->table('merchants_category') . ' WHERE cat_id = \'' . $cat_id . '\'');
}
...
{
	if (($table != '') && ($name != '')) {
		$field = mysql_query('Describe ' . $table . ' ' . $name);
...
{
	$sql = 'SELECT region_id, region_name, parent_id FROM ' . $GLOBALS['ecs']->table('region') . ' WHERE region_id = \'' . $region_id . '\'';
	return $GLOBALS['db']->getRow($sql);
}
...
{
	$sql = 'SELECT rw.region_id, rw.region_name, wa.attr_price, wa.id FROM ' . $GLOBALS['ecs']->table('region_warehouse') . ' AS rw' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('warehouse_attr') . ' AS wa on rw.region_id = wa.warehouse_id AND wa.goods_id = \'' . $goods_id . '\' AND wa.goods_attr_id = \'' . $goods_attr_id . '\'' . ' WHERE rw.parent_id = \'' . $parent_id . '\'';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
...
function get_main_order_goods_info($order_id = 0, $type = 0)
{
	...
	$sql = 'SELECT og.*, g.goods_weight as goodsWeight, g.is_shipping ' . $select . ' FROM ' . $GLOBALS['ecs']->table('order_goods') . ' as og, ' . $GLOBALS['ecs']->table('goods') . ' as g' . ' WHERE og.goods_id = g.goods_id AND og.order_id = \'' . $order_id . '\'';
	$res = $GLOBALS['db']->getAll($sql);
	$arr = array();
	...
	foreach ($newInfo as $key => $info) {
		...
		do {
			...
			if (($GLOBALS['_CFG']['sms_order_placed'] == '1') && ($sms_shop_mobile != '')) {
				...
				else if ($GLOBALS['_CFG']['sms_type'] == 0) {
					include_once 'includes/cls_sms.php';
					$sms = new sms();
					$msg = ($row['pay_status'] == PS_UNPAYED ? $GLOBALS['_LANG']['order_placed_sms'] : $GLOBALS['_LANG']['order_placed_sms'] . '[' . $GLOBALS['_LANG']['sms_paid'] . ']');
