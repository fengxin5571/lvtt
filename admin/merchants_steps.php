...
function steps_process_title_list($id)
{
	...
	if ($result === false) {
		...
		$filter = page_and_size($filter);
		$sql = 'SELECT tid, fields_steps, fields_titles, titles_annotation, fields_special, special_type ' . ' FROM ' . $GLOBALS['ecs']->table('merchants_steps_title') . $ex_where . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
		$filter['keywords'] = stripslashes($filter['keywords']);
		set_filter($filter, $sql);
	}
	else {
