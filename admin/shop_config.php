...
function get_settings($groups = NULL, $excludes = NULL)
{
	...
	$group_list = array();
	$code_arr = array('shop_logo', 'no_picture', 'watermark', 'shop_slagon', 'wap_logo', 'two_code_logo', 'ectouch_qrcode', 'ecjia_qrcode', 'index_down_logo', 'site_commitment');
	...
	foreach ($_FILES as $code => $file) {
		if ((isset($file['error']) && ($file['error'] == 0)) || (!isset($file['error']) && ($file['tmp_name'] != 'none'))) {
			if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
				...
				$code_store_dir = array('ecjia_qrcode', 'ectouch_qrcode', 'index_down_logo', 'site_commitment');
