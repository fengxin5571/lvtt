	...
	if (($_REQUEST['act'] == 'update') || ($_REQUEST['act'] == 'update_self')) {
		...
		if ($ru_id) {
			...
			if ($GLOBALS['_CFG']['sms_type'] == 0) {
				...
				$str_centent = array('seller_name' => htmlspecialchars($admin_name), 'seller_password' => htmlspecialchars(trim($_POST['new_password'])), 'current_admin_name' => $current_admin_name, 'edit_time' => local_date($GLOBALS['_CFG']['time_format'], gmtime()), 'shop_name' => $_CFG['shop_name'], 'seller_name' => $shop_name, 'mobile_phone' => $shopinfo['mobile']);
