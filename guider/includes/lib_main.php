 ...
 * 
 */
function set_admin_session($guide_id, $guidename,$last_time)
{
	$_SESSION['guide_id'] = $guide_id;
	$_SESSION['guide_name'] = $guidename;
	$_SESSION['guide_last_check'] = $last_time;
}
...
function get_insert_region($regionId, $ecs_region)
{
	foreach ($ecs_region as $row) {
		...
		if (0 < $res) {
			...
			$other = array('regionId' => $row['region_id'], 'parent_id' => $regionId, 'region_name' => $row['region_name'], 'region_type' => $row['region_type'], 'agency_id' => $row['agency_id']);
...
{
	if (0 < $ru_id) {
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('category') . ' AS c, ' . $GLOBALS['ecs']->table('merchants_category') . ' AS mc' . ' WHERE c.parent_id = \'' . $parent_cat . '\' AND c.cat_name = \'' . $cat_name . '\' AND c.cat_id<>\'' . $exclude . '\' AND c.cat_id = mc.cat_id AND mc.user_id = \'' . $ru_id . '\'';
...
function return_order_list()
{
	...
	if ($result === false) {
		...
		$sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid,' . '(SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og WHERE og.order_id = o.order_id LIMIT 1) AS ru_id, ' . 'o.pay_status, o.consignee, o.email, o.tel, o.extension_code, o.extension_id,' . 'r.ret_id ,r.rec_id, r.address , r.back , r.exchange ,r.attr_val , r.cause_id , r.apply_time , r.should_return , r.actual_return , r.remark , r.address , o.sign_time ,r.return_status , r.refound_status , ' . ' r.return_type, r.addressee, r.phone, r.return_sn, ' . '(' . order_amount_field('o.') . ') AS total_fee, ' . 'IFNULL(u.user_name, \'' . $GLOBALS['_LANG']['anonymous'] . '\') AS buyer ' . 'FROM ' . $GLOBALS['ecs']->table('order_return') . ' AS r ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('order_info') . ' AS o ON r.order_id = o.order_id ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('users') . ' AS u ON u.user_id=o.user_id  ' . $where . $where_store . ' GROUP BY r.ret_id ORDER BY ' . $filter['sort_by'] . ' ' . $filter['sort_order'] . ' ' . ' LIMIT ' . (($filter['page'] - 1) * $filter['page_size']) . ',' . $filter['page_size'];
